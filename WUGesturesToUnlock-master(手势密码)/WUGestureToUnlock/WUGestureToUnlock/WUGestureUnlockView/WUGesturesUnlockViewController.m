...
- (void)createGesturesPassword:(NSMutableString *)gesturesPassword {
    ...
    if (self.lastGesturePassword.length == 0) {
        ...
        if (gesturesPassword.length <4) {
            ...
            [self restartUnlockViewWithTime:2];
            return;
        }
        ...
        [self restartUnlockViewWithTime:0];
...
- (IBAction)resetGesturePassword:(id)sender {
    ...
    self.resetGesturesPasswordButton.hidden = YES;
    [self.gesturesUnlockView restartUnlockView];
...
#pragma mark  ---Aimi新增----
//根据一定的延迟时间将手势密码的界面重置
-(void)restartUnlockViewWithTime:(int)t{
    __block WUGesturesUnlockViewController *weakSelf = self;
    dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(t/*延迟执行时间*/ * NSEC_PER_SEC));
    
    dispatch_after(delayTime, dispatch_get_main_queue(), ^{
        [weakSelf.gesturesUnlockView restartUnlockView];
    });
}
